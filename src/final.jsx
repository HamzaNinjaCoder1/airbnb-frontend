...
function FinalStep({ onSaveAndExit, onNext, onBack }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => setProgress(p => Math.max(p, 71)), 60);
        ...
        <div className="min-h-screen bg-white">
            <div className="flex items-center justify-between px-8 py-4">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={onSaveAndExit} className="px-4 py-2 text-sm text-gray-800 border border-gray-300 rounded-full hover:bg-gray-100 font-semibold">Save & exit</button>
                </div>
            </div>
            ...
            <div className="max-w-[68rem] mx-auto px-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[70vh]">
                    <div className={`transition-all duration-700 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                        <div className="text-lg text-gray-800 font-semibold mb-2">Step 3</div>
                        <h1 className="text-[3rem] font-semibold text-gray-900 leading-tight mb-4">Finish up and publish</h1>
                        <p className="text-gray-800 text-[1.05rem]  leading-relaxed">
                            Finally, you'll choose booking settings, set up pricing, and publish your listing.
                        </p>
                    </div>
                    <div className={`transition-all duration-700 delay-200 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="fixed left-0 right-0 bottom-0 bg-white border-t">
                ...
                <div className="flex items-center justify-between px-8 py-4">
                    ...
                    <button onClick={onNext} className="px-8 py-3 bg-black text-white rounded-lg hover:bg-gray-900 transition-colors">Next</button>
                </div>
            </div>
        </div>
    );
