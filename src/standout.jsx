...
function StandOut({ onSaveAndExit, onNext, onBack, isSaving, hostId, progress, setProgress }) {
    ...
    useEffect(() => {
        const t = setTimeout(() => setProgress((prev) => Math.max(prev, 35)), 50);
        return () => clearTimeout(t);
    }, []);

    useEffect(() => {
        const r = setTimeout(() => setRevealed(true), 40);
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className={`flex items-stretch min-h-[calc(100vh-160px)] px-8 transition-all duration-700 ease-out ${revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
                <div className="w-[36%] flex items-center ml-48">
                    <div className="max-w-2xl">
                        <div className="text-lg text-gray-800 font-semibold mb-2">Step 2</div>
                        <h1 className="text-[2.9rem] font-semibold text-gray-900 leading-tight mb-4">Make your place stand out</h1>
                        <p className="text-gray-600 text-[1.05rem] font-medium leading-relaxed">
