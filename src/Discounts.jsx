...
const DiscountCard = ({ percent, title, subtitle, checked, onChange }) => {
    ...
    useEffect(() => {
        if (checked) {
            setShowBump(true);
            const t = setTimeout(() => setShowBump(false), 400);
            return () => clearTimeout(t);
        }
    }, [checked]);

    return (
        <div className={`flex items-center justify-between p-6 rounded-xl border border-gray-300 bg-[#F7F7F7]`}>
            <div className="flex items-center gap-4">
                <div className={`w-16 h-11 rounded-lg border flex items-center justify-center font-semibold border-gray-300 transition-all duration-300 ${showBump ? 'scale-105' : 'scale-100'}`}>
                    <span className="text-base md:text-lg">{showBump ? `${percent + 1}%` : `${percent}%`}</span>
                </div>
                <div>
            ...
            <button
                ...
                className={`w-6 h-6 rounded-sm border flex items-center justify-center transition-all duration-200 ${checked ? 'bg-black border-black' : 'border-gray-400 bg-white'} hover:scale-105`}
            >
                {checked && (
...
const Discounts = () => {
    ...
    const startingProgress = location.state?.progress || 95;
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => setProgress(p => Math.max(p, 96)), 60);
        ...
        try {
            if (hostId) {
                const discountSettings = {
                    new_listing_discount: newListing,
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            const urlWithHostId = hostId ? `/final-details?hostId=${hostId}${listingId ? `&listingId=${listingId}` : ''}` : '/final-details';
    ...
    return (
        <div className="min-h-screen bg-white overflow-y-auto">
            ...
            <div className="max-w-[45rem] mx-auto px-6 mt-6 pb-32">
                <div className={`transition-all duration-700 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <h1 className="text-[2rem] font-semibold text-gray-900">Add discounts</h1>
                    <p className="text-gray-600">Help your place stand out to get booked faster and earn your first reviews.</p>
                </div>
                ...
                <div className="mt-6 space-y-4">
                    ...
                    <DiscountCard
                        ...
                        subtitle="For stays booked 14 days or less before arrival"
                        checked={lastMinute}
