...
function TitleStep({ progress, setProgress }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => setProgress(p => Math.max(p, 53)), 60);
        return () => { clearTimeout(m); clearTimeout(t); };
    }, []);

    const remaining = 50 - title.length;
    const isNextEnabled = title.trim().length > 0 && title.length <= 50;
    ...
    const handleNext = async () => {
        ...
        try {
            if (hostId) {
                if (editMode) {
                    // In edit mode, just update the title and return to listings
                    await saveListingProgress({ 
                        title: title.trim(),
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="max-w-[48rem] mt-10 mx-auto px-6">
                <div className={`transition-all duration-700  ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <div className='leading-tight'>
                        <h1 className="text-[2.3rem] font-semibold text-gray-900 mt-4">Now, let's give your earth home a title</h1>
                    <p className="text-gray-600 text-[1.2rem] font-semibold mt-2">Short titles work best. Have fun with it—you can always change it later.</p>
                    </div>
                    ...
                    <div className="mt-8">
                        <textarea
                            ...
                            className="w-full h-44 border border-gray-800 text-gray-700 rounded-xl p-4 outline-none focus:ring-2 focus:ring-black resize-none text-2xl font-semibold"
                            placeholder=""
                            maxLength={50}
                        />
                        <div className="text-[1.05rem] font-semibold text-gray-500 mt-2">{50 - title.length}/50</div>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed left-0 right-0 bottom-0 bg-white border-t">
                ...
                <div className="flex items-center justify-between px-8 py-4">
                    <button onClick={() => {
                        const urlWithHostId = hostId ? `/aboutplace/photos?hostId=${hostId}${listingId ? `&listingId=${listingId}` : ''}` : '/aboutplace/photos';
                        navigate(urlWithHostId, { state: { progress } });
