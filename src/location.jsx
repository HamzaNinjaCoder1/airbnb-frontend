...
function LocationStep({ onSaveAndExit, onNext, onBack, isSaving, hostId, onLocationDataChange, progress, setProgress, editMode }) {
    const { isAuthenticated } = useAuth();
    const navigate = useNavigate();
    const location = useLocation();
    if (!isAuthenticated) return null;

    const defaultAddress = 'The Garden Spice, Canal Rd, adjacent Green Forts 2, Jhelum Block Green Forts 2, Lahore, 54000';
    const [query, setQuery] = useState(defaultAddress);
    const [iframeSrc, setIframeSrc] = useState(`https://www.google.com/maps?q=${encodeURIComponent(defaultAddress)}&output=embed`);
    ...
    useEffect(() => {
        const t = setTimeout(() => setProgress((prev) => Math.max(prev, 24)), 50);
        return () => clearTimeout(t);
    }, []);
    // Function to extract location data from address
    const extractLocationData = (address) => {
        // Simple parsing - in a real app, you'd use a geocoding service
        ...
        return {
            ...
            city: city,
            country: country,
            latitude: null, // Would be filled by geocoding service
            longitude: null // Would be filled by geocoding service
        };
    };
    ...
    useEffect(() => {
        ...
        const handle = setTimeout(() => {
            ...
            const newLocationData = extractLocationData(query);
            setLocationData(newLocationData);
            
            // Notify parent component of location data change
            if (onLocationDataChange) {
                onLocationDataChange(newLocationData);
            }
        }, 500);
        return () => clearTimeout(handle);
    }, [query, onLocationDataChange]);
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="max-w-[45rem] mx-auto px-6">
                <h1 className="text-3xl font-semibold text-center -mt-2">Where's your place located?</h1>
                <p className="text-gray-600 text-center text-[1.05rem] mt-2 mb-6">Your address is only shared with guests after they’ve made a reservation.</p>

                <div className="relative rounded-2xl overflow-hidden shadow-sm border">
                    <div className="absolute right-6 top-6 z-10 w-[45%] max-w-[520px]">
                        <div className="flex items-center gap-2 bg-white rounded-full px-5 py-4 shadow-md border">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 21l-4.35-4.35" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="1.5"/></svg>
                            <input
                              type="text"
            ...
            <div className="fixed left-0 right-0 bottom-0 bg-white border-t">
                ...
                <div className="flex items-center justify-between px-8 py-4">
                    ...
                    <button
                        disabled={!query || query.trim().length < 3 || isSaving}
                        aria-disabled={!query || query.trim().length < 3 || isSaving}
                        onClick={() => {
                            if (!query || query.trim().length < 3) return;
                            onNext();
                        }}
                        className={`px-8 py-3 rounded-lg transition-colors ${query && query.trim().length >= 3 && !isSaving ? 'bg-black text-white' : 'bg-black/30 text-white/70 cursor-not-allowed'}`}
                    >
