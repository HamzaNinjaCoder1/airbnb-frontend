...
function DescriptionStep() {
    ...
    const mapping = {
        rare: 'This place to stay is truly one of a kind.',
        rustic: 'Adventure awaits you in this rustic getaway.',
        nature: 'Discover the gorgeous landscape that surrounds this place to stay.',
        memorable: 'You’ll treasure your time at this memorable place.',
        romantic: 'Take in the special details of this romantic place.',
        historic: 'Step back in time when you stay at this unique and historic place.'
    };

    const fragments = {
        rare: 'truly one of a kind',
        rustic: 'a rustic getaway full of adventure',
        nature: 'surrounded by gorgeous landscapes',
        memorable: 'a memorable place you’ll treasure',
        romantic: 'a romantic place with special details',
        historic: 'a unique, historic stay'
    };

    const selectedKeys = (location?.state?.selected && Array.isArray(location.state.selected)) ? location.state.selected : [];

    const buildCombinedDescription = (keys) => {
        if (!keys || keys.length === 0) return '';
        if (keys.length === 1) return mapping[keys[0]] || '';
        const [a, b] = keys.slice(0, 2);
        const partA = fragments[a] || '';
        const partB = fragments[b] || '';
        if (partA && partB) {
            return `This place to stay is ${partA} and ${partB}.`;
        }
        return `${mapping[a] || ''} ${mapping[b] || ''}`.trim();
    };

    const initialText = buildCombinedDescription(selectedKeys);

    const [text, setText] = useState(initialText);

    useEffect(() => {
        const m = setTimeout(() => setIsMounted(true), 0);
        const t = setTimeout(() => setProgress(p => Math.max(p, 65)), 60);
        return () => { clearTimeout(m); clearTimeout(t); };
    }, []);

    const isNextEnabled = text.trim().length > 0 && text.length <= 500;
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="max-w-[50rem] mt-6 mx-auto px-6">
                <div className={`transition-all leading-tight duration-700 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <h1 className="text-[2.3rem] font-semibold text-gray-900">Create your description</h1>
                    <p className="text-gray-500 text-[1.1rem] font-medium mt-2">Share what makes your place special.</p>

                    <div className="mt-10">
                        <textarea
                            value={text}
                            onChange={(e) => setText(e.target.value.slice(0, 500))}
                            className="w-full h-56 border border-gray-300 rounded-xl p-4 outline-none focus:ring-2 focus:ring-black resize-none text-2xl font-semibold"
                            maxLength={500}
                        />
                        <div className="text-[1.05rem] font-medium text-gray-500 mt-2">{text.length}/500</div>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed left-0 right-0 bottom-0 bg-white border-t">
                ...
                <div className="flex items-center justify-between px-8 py-4">
                    <button onClick={() => {
                        const urlWithHostId = hostId ? `/aboutplace/highlights?hostId=${hostId}${listingId ? `&listingId=${listingId}` : ''}` : '/aboutplace/highlights';
                        navigate(urlWithHostId, { state: { progress } });
